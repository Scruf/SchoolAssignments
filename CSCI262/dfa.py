...
q0 = _file_content[2]
if q0 not in states:
	print("Start state %s not in states %s"%(q0,' '.join(states)))
	sys.exit(0)
...
for state in set_of_accepted_states:
	if state not in states:
		print("Accepted state %s not in set of states %s"%(state,' '.join(set_of_accepted_states)))
		sys.exit(0)
